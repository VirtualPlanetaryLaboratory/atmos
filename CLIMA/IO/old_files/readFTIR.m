...
v= [5000:-10:.1]; % wavenumber (inverse centimeters) (5 microns to 100,000 microns) This is why 
...
C(:,3) = Fluxbin;
diary on
        disp('  Interval     *Wavenumber(inv cm)     OLR(W/m^2/cm^-^1)')
for j = 1:55
        fprintf('%4.0f              %5.0f                   %g',C(j,1), C(j,2), C(j,3))
        fprintf('\n')
end
disp('*Note that these OLR data are plotted against the midpoints of their intervals')
diary off
