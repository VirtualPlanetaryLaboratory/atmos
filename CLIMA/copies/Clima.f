...
c     PARAMETER(NF=55, NA=1, NLAYERS=51, NZ=64)
      ...
      parameter(nrow=11)			
c-rr  Changed STARR to be able to hold 5 characters 3/29/11
      CHARACTER*5 :: STARR
      CHARACTER*11 :: AA
     ...
     &  dt(ND),DIVF(ND),CPNT(ND),TCOOL(ND),THEAT(ND),FLAGCONVEC(ND)
...
c      OPEN(unit=89, file=DIRINOUT//'/TPRIND.dat')
...
c Nitrogen mixing ratio  
      FN2 = 1. - FO2 - FAR - FCO2 - FCH4 - NO2
c Molecular weight of the atmosphere
      DM = 28.*FN2 + 32.*FO2 + 40.*FAR + 44.*FCO2 + 16.*FCH4 + 46.*NO2
...
C Heat capacity calculation
      DO J=1,ND-1
c      CPCO2 = 7.7 + 5.3E-3*T(J) - 8.3E-7*T(J)*T(J) 
c-rr The new CPCO2 parametrization 3/19/11    
      CPCO2 = 5.89 + 6.06E-3*T(J) + 2.39E-5*T(J)*T(J) 
     & -3.44E-8*T(J)*T(J)*T(J)  
      CPN2 = 6.76 + 6.06E-4*T(J) + 1.3E-7*T(J)*T(J)
c      CPO2 = 8.27 + 2.58E-4*T(J) - 1.877E5/T(J)/T(J)
c-rr The new CPO2 parametrization 3/27/11
      CPO2 = 7.47 -4.84E-3*T(J) + 1.38E-5*T(J)*T(J) 
     & -8.73E-9*T(J)*T(J)*T(J) - 1.76E-9/T(J)/T(J)
      CPO2 = AMAX1(CPO2,CPN2)
c Total heat capacity     
C     CPN = FCO2*CPCO2 + FN2*CPN2 + FO2*CPO2 + FAR*4.97 +FCH4*8.3
      CPN = FCO2*CPCO2 + FN2*CPN2 + FO2*CPO2 + FAR*4.97 +FI(3,J)*8.3
...
C 571   CONTINUE
      ...
      WRITE(98,*)   
      WRITE(98,555) SOLCON,FCH4,FCO2,FO2,FN2,IO3,IUP
...
c     &  TCOOL(J)
 ...
 467  FORMAT(1PE10.4,2X,1PE10.4,1X,1PE10.4,3X,1PE11.4,3X,1PE11.4,
     & 3X,1PE11.4,3X,1PE11.4,3x,1pe11.4) 
      END DO
c       close(89)
       STOP
