...
C
   ...
   19   FORMAT(/1X,1PE10.4)
         ...
         NPR3=6
         END IF
C-rr	 Intervals 1-21 use Karkoshka's 4 term CH4 alphas from "nearIrexpsums.pdat" while intervals 22-38 use new CH4 alphas
C        from the 6 term data statement in readsol (CH4 is gas 3). All 38 intervals use Tom Ackerman's 4 term alphas for    
C        gases 1 and 2 (solar_data_38.pdat).
...
c-rr	      Putting new CPCO2 parametrization here. Why is there no FCH4?
	      CPCO2 = 5.89 + 6.06E-3*TF(J) + 2.39E-5*TF(J)*TF(J) 
     &        -3.44E-8*TF(J)*TF(J)*TF(J)  
...
c	      Putting new CPO2 parametrization 3/28/11
	      CPO2 = 7.47 -4.84E-3*TF(J) + 1.38E-5*TF(J)*TF(J) 
     &        -8.73E-9*TF(J)*TF(J)*TF(J) - 1.76E-9/TF(J)/TF(J)
