      ...
      SUBROUTINE IRM(T,PF,P,CGAS,FI,IMET)
c
...
c       ENDIF
	...
	DO 4 K3=1, 6 ! This is methane loop
c       DO 5 K0=1, 6
...
c       if ((IL.eq.iil).and.(I.eq.22)) print *, 'CIA=',CPRW(IL,I)
c     &  , 'LAMB=',I
       IF(I .EQ. 38)THEN
          CPRW(IL,I) = 4.0E-8  !Rewriting CIA at intervals 37 and 40 with appropriate CIA at 2.3 and 1.73 microns, respectively(Tsang et al. 2008).
       ELSEIF (I.EQ.41)THEN
...
c         TPRIND(IL) = CPR(I)*PPE*TPE*CGAS1
...
c          if ((IL.eq.100).and.(K1.eq.1).and.(K2.eq.1).and.(I.eq.22))then
