...
c
c jfk Watch out! When this subroutine was called by Clima, the second
c     variable in the call sequence was PF, not P. So, these pressures
c     should be the pressures at the boundaries of the layers, not at
c     the layer midpoints. This subroutine needs to be recoded to 
c     reflect this, as there could be some errors that have resulted.
c     This comment was added 6/25/08.
c
...
c     statement. Actually, this statement may remain once the code is
c     corrected.)
      DIMENSION PF(ND)
...
C       np = 1.E+1
c
c jfk  Temporary: Set PF = P because P was misdefined earlier
      DO J=1,ND
      PF(J) = P(J)
      END DO
