...
! Intiliazing Kappalayer, temperature, and FX indices    4/25/2012 c-rr
        DO IL = 1, NLAYERS
          DO J = 1,IK
            DO I = 1,NF
...
!        print *, 'here is where the kmatrix would go'
...
c      CALL IREXPSUMS(WEIGHT,xkappa)
     ...
     &  kmatrix_irco2, kmatrix_irh2o) ! Outputting kmatrix_irco2 and kmatrix_irh2o because these quantities are not global, unlike in solar (where they are put in common blocks. 8/27/2012.
!       print *, 'called interpir'
 
       CALL INTERPCO2CIA(T(IL),IL,MS(IL),MS1(IL),FX(IL))
!       print *, 'called interpco2cia'
!         KMS(IL)  = MS(IL)
!         KMS1(IL) = MS1(IL)
...
!       print *, 'called interph2n2cia'
       
       CALL INTERPO2CIA(T(IL),IL,MSO(IL),MSO1(IL),FXO(IL))
!       print *, 'called interpo2cia'

       CALL INTERPH2H2CIA(T(IL),IL,MSHH(IL),MSHH1(IL),FXHH(IL))
!       print *,' called interph2h2cia'

!        print *, 'MS=', MSO(IL), 'MS1=', MSO1(IL), T(IL), IL
!        pause
...
!----------------BPS WATER CONTINUUM 8/30/2012
     ...
     &		+ RADFLD*Bsir(i)) ! Self broadening coefficient with radiation field and temperature dependence included (cm^2/molecule)
		...
		TAUCONTIN(IL) = ABSCONT*CGAS(IL,6) !optical depth
...
!	        endif
         ...
         TAUGCH4(IL) = KAPPALAYER(I,K3,3,IL)*CGAS(IL,2)!*2.687E24   ! Activate this for methane
!           if(IL .eq. 1)then
!            if(NST==5)then
...
c-rr	This the H2-N2 CIA loop calculation 5/29/2012
        H2N2CIAMS1L = alog(H2N2CIA(MSH1(IL),I))
        H2N2CIAMSL  = alog(H2N2CIA(MSH(IL),I))
...
c-rr	This the H2-H2 CIA loop calculation 7/02/2012
        H2H2CIAMS1L = alog(H2H2CIA(MSHH1(IL),I))
        H2H2CIAMSL  = alog(H2H2CIA(MSHH(IL),I))
...
c-rr	This is the O2 CIA loop calculation 6/17/2012
        O2O2CIAMS1L = alog(O2O2CIA(MSO1(IL),I))
        O2O2CIAMSL  = alog(O2O2CIA(MSO(IL),I))
...
!           if ((K1.eq.1).and.(I.ge.21).and.(I.le.28))then
!            print *, O2O2FIN(IL,I), TAUO2O2(IL), FO2, FNC(IL), CGASO2
!     &      , CGAS1, TAUGIR(IL), T(IL), IL
!            pause
!         endif         
...
c      TAUGIR(IL) =TAUGIR(IL) + TAUGOZ(IL)
      TAUGIR(IL) = TAUGCH4(IL)+TAUGH2O(IL)+ TAUGCO2(IL) + TPRIND(IL)  ! We combined CO2 and H2O tau 3/21/2012
     ...
     & + TAUO2O2(IL)+TAUH2H2(IL)
      ...
      DO IL=1, NLAYERS
	   ...
	   FCO2 = FI(2,IL)  ! Needed for rayley
          ...
          OMG0IR(IL) = AMAX1(OMG0IR(IL),1.E-5)  ! changed lower limit of single-scattering albedo to 1e-12 (default 1.e-5) c-rr 4/30/2012
      ENDDO
