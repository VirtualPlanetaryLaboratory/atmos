...
!       PARAMETER(NSTARS=8)  ! No. of stars in STELLAR_SPECTRA.pdat
!-RKK!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
c-rr  3/29/11.. Adding GNEW 10/12/2012
     ...
     &  CPN(ND), CPNT(ND), CPH2(ND)  ! Added CPH2 5/31/2012 c-rr
      ...
      DATA BETA/ND*1./
!      REAL TG1(39) !created new temperature array 4/29/11
      DATA BETH2O/1100*0./
      DATA BETCO2/1100*0./
...
!     & 600., 800., 1000., 1200., 1220., 1240.,1260., 1280., 1300.,
!     & 1320., 1340., 1360., 1380., 1400., 1500.,1600./ 
...
!       pmass = 1.000d0
!       do while(pmass <=1.000d0)

!============================================================================
! The mass-radius relations for terrestrial planets is taken from:
! "Exoplanets" (2010) book. University of Arizona Press - Space Science Series. 
!  Chapter on "Terrestrial Planet Interiors" by Sotin, Jackson & Seager.
!  Equation (12) and table 5.
  
!          if( (pmass >=0.01d0) .and. (pmass < 1.0d0))then
!              rad = pmass**0.306
!          else
!              rad = pmass**0.274
!          endif
!============================================================================
!          G   = 6.67E-8*pmass*5.9742E+27/(rad*6378.1E+5)**2
!          PG0 = 1.0204E-3 * G  ! For Earth PG0/G = 1.0204e-3
...
!-RKK
!
! To calculate Inner HZ boundaries for all types of stars, loop over
! 'STARHZ' variable.

      WRITE(244,1120)
1120  FORMAT('TEFF',5x,'SEFF',3x,'SQSEFF', 3x,'ALBP',3x,'FTSOL')
...
c-rr Noncondensible molecular weight of the atmosphere when H2O is convecting only (no CO2 condensation) 5/3/2011
      DM = 44.*FCO2 + (1.-FCO2)*DM2
       ...
       print *,'For star ',STARR,JJ

       WRITE(24,*)STARR,JJ
       WRITE(24,*)'TGO        SEFF         PALB    FH2O
     &              FTIR(1)         FTSO(1)'
       WRITE(24,*)'=================================================='

       print *,'TGO        SEFF         PALB    FH2O
     &              FTIR(1)         FTSO(1)'
       print *,'=================================================='
...
!        TG1 = TG0 ! assigning the original TG0 read from input_clima as first value in PG1 pressure array
        DO while(TG0 <=2201.0d0)  ! to do the inner edge calculation for all 16 temperatures all at once
...
!-RKK MAY 29 2011.
!
! The following condition is to write an output file called 
! 'waterloss_IHZ.dat'. As the name explains, this file contains the data for
! waterloss limit of the IHZ. According to Kasting et al.(1993) Fig. 3 
! (Eq. 8), this limit happens when water mixing ratio, FH20, ~ 3x10^-3. 
! Hence, at that limit, IHZ boundary normalized to the luminosity of the star 
! is  calculated. So to obtain IHZ in AU, multiply the last column in this file
! with the SQRT(LSTAR/LSUN).
!

       IF( (FI(1,1) >9.00E-4 ) .and. (FI(1,1) < 7.0E-3) )THEN
          WRITE(244,1110)Teff,SEFF,SQRT(1.0/SEFF),ALBP,FTSO(1)/1000.

       ENDIF
1110   FORMAT(F6.1,2X,F6.4,2X,F6.4,2X,F6.4,4X,F7.2)
!========================================================================
        ...
        write(24,7777) TG0,SEFF,ALBP,FI(1,1), abs(FTIR(1))/1000, 
     &   FTSO(1)/1000
        print 7777,TG0,SEFF,ALBP,FI(1,1),abs(FTIR(1))/1000
!        pause
...
!       ENDIF 
        ...
        if(TG0 <600.0d0)then
            TG0 = TG0 + 20.0d0
              ...
              if((TG0 >=600.0d0)  .and. (TG0 <1000.0d0))then
                TG0 = TG0 + 200.0d0
              
              else 
                if((TG0 >=1000.0d0)  .and. (TG0 <=2300.0d0))then
                    TG0 = TG0 + 50.0d0
                endif
               endif
        endif
...
!       if((TG0 >=600.0d0)  .and. (TG0 <1300.0d0))then
!          TG0  = TG0 + 100.0d0
!       endif

!        if((TG0 >=1300.0d0)  .and. (TG0 <1360.0d0))then
!          TG0 = TG0 + 20.0d0
!        endif
!!        if((TG0 >=1360.0d0)  .and. (TG0 <1380.0d0))then
!          TG0 = TG0 + 8.0d0
!        endif
!        if(TG0 >=1380.0d0)then
!          TG0 = TG0 + 200.0d0
!        endif



       ENDDO !   ENDDO FOR TG0 LOOP
...
!       pmass = pmass + 0.50
!       ENDDO   !ENDDO for planetary mass
