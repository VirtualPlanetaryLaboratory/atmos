...
c     PARAMETER(NF=55, NA=1, NLAYERS=51, NZ=64)
      ...
      PARAMETER(NSOL=38, NGS=5, IK=16)  ! Added IK=16 parameter and NGS is 5 now, 3/26/2012
      parameter(nrow=11)			
...
c     COMMON/CBLOK/FO2,FN2,FCO2,FAR,FCH4
     ...
     & kappa_sol(nsol,7,9,6,6,16),BETAO3(14), ccc(14), d(14), BETAO2
...
c-rr !3/23/11 put CIA matrix in IRDATA
      COMMON/VARIR/kappa_ir(55, 7, 9, 6, 6, 16), 
     & weightco2_h2oIR(16)! Added kappa matrix in IR for kpsectrum mixed CO2/H2O coefficients 3/20/2012  
...
c-rr  3/29/11
      COMMON/CPHEAT/CPO2(ND),CPCO2(ND), CPN2(ND), CPH2O(ND),CPN(ND), 
     &  CPNT(ND)
...
C  INPUT FILES
      ...
      OPEN (unit=5, file=DIRDATA//'/solar_38.dat', status='old')
      OPEN (unit=6, file=DIRDATA//'/ir_55.dat', status='old')
...
c     OPEN(unit=10, file=DIRDATA//'/GJ581.txt',status='old')
...
c  IMPORTANT Files these are read in the subroutine pickstar
c        10     DIRDATA/STELLAR_SPECTRA.pdat (contains the Sun and any other stars)
...
c    File formerly called photo_input.dat
      OPEN (unit=16,file= DIRINOUT//'/fromClima2Photo.dat')
...
c*******Changed for now*********
      ...
      READ(1,*) AA,CO2MAX
      READ(1,*) AA, IMET	! Methane flag (0 or 1)
      READ(1,*) AA, nga
...
c Nitrogen mixing ratio  
      FN2 = 1. - FO2 - FAR - FCH4 - FNO2   

c Initial non-condensible mixing ratio (used for write statement in output file) 5/14/2011    
      FNCI = FN2 + FO2 + FAR + FCH4+ FNO2

c-rr Noncondensible molecular weight of the atmosphere when CO2 is condensing (for a colder planet)	  5/3/2011
      DM2 = 28.*FN2 + 32.*FO2 + 40.*FAR + 16.*FCH4 + 46.*FNO2
...
c Constructing temperature and water profiles in case they are not provided 
       IF(IUP.EQ.1) THEN
          JCOLD = 1
          CALL PROFILE(TSTRAT,P,T,DZ,FSAVE,FCO2V,BETA,JCOLD,IDRY, 
     &    FLAGCONVEC)           
       ENDIF
...
c-rr   Created IRM.F (IR clone with methane loop turned on). When there is methane call IRM instead of IR. 5/2/2011 
       ...
       CALL IR(T,PF,P,CGAS,FI)
       print *,'out of IR'

       ELSE IF(IMET.eq.1) THEN
       CALL IRM(T,PF,P,CGAS,FI,IMET)
       ENDIF
c      else
...
c Total heat capacity     
       ...
       CPN(J) = FI(1,J)*CPH2O(J)+ FI(2,J)*CPCO2(J) + FNC*CPNC
...
C 571   CONTINUE
      ...
      WRITE(98,*)   
      WRITE(98,555) SOLCON,FCH4*FNCI,FCO2,FO2*FNCI,FN2*FNCI,IO3,IUP
 555  format(1x,"Solar Constant= ",F6.2,3x,"F_CH4= ",1pe10.4,2x,
...
c     & DELT(J),TOLD(J),FI(1,J),HEATNET(J),TCOOL(J),THEAT(J)
     ...
     & DELT(J),TOLD(J),FI(1,J),FSAVE(J),TCOOL(J),THEAT(J)
       ENDDO
      WRITE(98,1683)
      Write (98,1112)
 1112 FORMAT(/1x,'FCO2')
...
c-rr If cflag is 0 then do this....  5/3/2011
      DO J=ND-1,1,-1
       TA = 0.5*(T(J) + T(J+1))
       FH2O = 0.5 * (FI(1,J) + FI(1,J+1))
       FCO2J = 0.5* (FI(2,J) + FI(2,J+1))
       FNC = 1. - FH2O - FCO2J
       AM = 18.*FH2O + 44*FCO2J + DM2*FNC
       IF(NST.lt.1) AM = DM2     
       BMG = BKM/AM
