...
!note _ i haven't yet done the work to fully abstract particles in this subroutine.
...
c      integer wl
     ...
     4  TAUSHP(NZ)
      DIMENSION A(NZ2),B(NZ2),D(NZ2),E(NZ2),Y(NZ2),FUP(NZP1),FDN(NZP1)
      DIMENSION W0P(NP),QEXT(NP),GFC(NZ),TAUSCAT_PART(NP,NZ),TAUP(NZ)
...
C   Particle 1 is sulfate, 2 is S8, 3 is HCAER
      W0P(1) = 1.
      W0P(2) = 0.5
      IF (WAV .GT. 3500.) W0P(2) = 1.    !ACK hardcoded grid
...
C-MC Here, we are including correct value of Qext and W0P for hydrocarbons
      IF (J .GE. 3) THEN   !hardcoded for HCAER and HCAER2
       QEXT(J) = QEXTT(LL,I,J)     
       W0P(J) = W0T(LL,I,J)                          
      ELSE
       QEXT(J)=2.  !for sulfate and S8 : valid for large particles only, but this is status quo for now... 
       !W0P set above
       GFT(LL,I,J)= 0.8  !for sulfate and S8, set assymetry factor to GP=0.8 (alt and wl independent   
      ENDIF

      TAUP1 = QEXT(J)*PI*RPAR(I,J)*RPAR(I,J)*AERSOL(I,J)*DZ(I)
      TAU_PART(J,I)=TAUP1  !particle extinction for each particle at each height  !not used, but might as well keep
      TAUSCAT_PART(J,I)=W0P(J)*TAUP1  !particle scattering for each particle at each height
      TAUSP(N) = TAUSP(N) + TAUSCAT_PART(J,I)    !TAUSP contains total particle scattering at each height
      TAUP(N) = TAUP(N) + TAUP1  !TAUP contains total particle extinction at each height
      enddo
      enddo
...
C   Avoid letting W0 equal exactly 1.
      DO  N=1,NZ
      TAU(N)=TAUG(N) + TAUP(N) !TAU is total extinction (Gas + Rayleigh + Particle)
      W0(N) = (TAUSG(N) + TAUSP(N))/TAU(N)  !w0 = total scattering/total extinction
      W0(N) = AMIN1(W0(N), 0.999)

      I= NZP1 - N  !GFT is still bottom to top, so needs a switch

      GPnew=0.0
      do k=1,np
c         if (k.ge.3) print *, LL,I,K,GFT(LL,I,K)
      GPnew=GPnew + GFT(LL,I,K)*TAUSCAT_PART(K,I)/(TAUSP(N) + TAUSG(N)) !asymmetry factors weighted by particle scattering/total scattering
      enddo   

      GT(N)=amin1(Gpnew,0.999)  !dont let assymetry factor get larger than 1
...
C
      ...
      WRITE(22,114) WAV,TAUCTSTR(NZP1)   !ACK - check LUN
 114  FORMAT(1X,F6.1,2X,1PE10.3)
      IF (WAV.EQ.1860.5 .OR. WAV.EQ.2010. .OR. WAV.EQ.2116.5 .OR.   !ACK - what are these hardcoded wavelengths???
     2    WAV.EQ.2211. .OR. WAV.EQ.2312.5 .OR. WAV.EQ.2516. .OR.
     3    WAV.EQ.3007.5 .OR. WAV.EQ.3900. .OR. WAV.EQ.4500.) THEN
      WRITE(20,106)WAV,U0,ALB    !ACK - check LUN
 106  FORMAT('# WAV = ',F6.1,2X,'U0 = ',F6.4,2X,'Rsfc = ',F5.3,
     ...
     4   'TAUSG(N)',3X,'TAUSP(N)',3X,'N')
      DO 113 N=1,NZ
      I = NZP1 - N
      WRITE(20,107) I,TAUCTSTR(N),TAU(N),W0(N),GT(N),TAUSG(N),TAUSP(N),N  !ACK - check LUN
 107  FORMAT(1X,I3,1P6E11.3,1X,I3)
 113  CONTINUE
      WRITE(20,105)TAUCTSTR(NZP1)   !ACK - check LUN
...
C   to be sure this only happens near the ground where AMEAN ~ 0.
      ...
      IF(AMEAN(N).LT.0.0)THEN
         WRITE(13,103) WAV,N,AMEAN(N)   !ACK - check LUN
...
C
     ...
     2    WAV.EQ.2211. .OR. WAV.EQ.2312.5 .OR. WAV.EQ.2516. .OR.
     3    WAV.EQ.3007.5 .OR. WAV.EQ.3900. .OR. WAV.EQ.4500.) THEN
      WRITE(21,108)WAV,U0,ALB  !ACK - check LUN
 108  FORMAT(/'# WAV = ',F6.1,2X,'U0 = ',F6.4,2X,'Rsfc = ',F5.3,
     2  /'#  Z',4X,'S(Z)',7X,'Y1',9X,'Y2',
     3  9X,'DIRECT',5X,'AMEAN',6X,'FUP',8X,'FDN',7X,'N')
      DO 111 N=1,NZ
      I = NZP1 - N
      WRITE(21,109) I,S(I),Y1(N),Y2(N),DIRECT(N),AMEAN(N), !ACK - check LUN
     2  FUP(N),FDN(N),N
 109  FORMAT(1X,I3,1P7E11.3,1X,I3)
 111  CONTINUE
      WRITE(21,112) DIRECT(NZP1),AMEAN(NZP1),FUP(NZP1),FDN(NZP1) !ACK - check LUN
