...
c READ IN TWO BODY, 3-PARAMETER REACTION RATES ! APL; many reactions could replace WEIRD with this !stb only for non KIDA
...
c       A(J,I) = 9.46E-34*EXP(480./T(I))*DEN(I)  ! NIST 05 low Temp in N2.  Its 2x bigger in H2 or O2  !stb different in version from apl, can't find reference for apl version while these values do appear on NIST
       A(J,I) = 6.0E-34*(300./T(i))**2.6 * DEN(I)  ! NIST 05 low temp in O2/N2 <--APL's version
      endif
!   O + O2 + M -> O3 + M
      if (CHEMJ(1,J).EQ.'O'.AND.CHEMJ(2,J).EQ.'O2') THEN
       A(J,I) = 5.12E-34*(298./T(i))**2.6 * DEN(I) * exp(40.5/t(i))  ! NIST 05 combination fit  !stb, reaction from apl
      endif
...
c       A(J,I) = 1.*2.2E-33*EXP(-1780./T(I))*DEN(I)  ! no information
...
!in reactions list as "WEIRD" but wasn't here...
     ...
     $    CHEMJ(3,J).EQ.'HSO') )THEN
      A(J,I) = 5.6E-12 *(T(I)/298.)**(-0.4)    ! Kasting 1990 <--there was a sign error in the exponent before
      endif
