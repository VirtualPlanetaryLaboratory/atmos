...
c-mc rate constant units are cm^3/mol/s
 ...
 667   FORMAT(60X,E9.2,3X,F8.2)            !for two body reaction rates
 668   FORMAT(60X,E9.2,3X,E9.2,X,2F6.2)   !for three body reaction rates
...
!            print 700,  alpha_0,beta_0,gamma_0,f_0,g_0
            do i=1,nz
              A(J,I)=alpha_0*((T(I)/300.)**beta_0)*EXP(-1.*gamma_0/T(I))    !two body reaction rates
            enddo
          else
        ...
        ! Read in 2BCRT reaction rates
          ...
          !read in Arhenius and Temperature factor (note TFAC contains the negative sign. not standard practice)
          !no "if (KIDA.eq.1)" needed here because the "old format" doesn't include 2BCRT.
          read (9,700) alpha_0,beta_0,gamma_0,f_0,g_0
          !print 700, alpha_0,beta_0,gamma_0,f_0,g_0
     ...
     $                  beta_inf,gamma_inf,f_inf,g_inf
            !if (j.eq.85) print 700, alpha_0,beta_0,gamma_0,f_0,g_0,alpha_inf,
!     $                 beta_inf,gamma_inf,f_inf,g_inf
            !multiply low density rate by 2.5 to account for CO2 rather than N2 as background gas (Nair, 94)
            if (PLANET .EQ. 'MARS') B=B*2.5
            do i=1,nz
     ...
     $                 beta_inf,gamma_inf,T(I),DEN(I))
            ...
            read(9, 668) B, C, D, E
          !  print 668, B, C, D, E
            if (PLANET .EQ. 'MARS') B=B*2.5      !multiply low density rate by 2.5 to account for CO2 rather than N2 as background gas (Nair, 94)
            do i=1,nz
              A(J,I)= TBDY(B,C,D,E,T(I),DEN(I))  !computed three body reaction rate
            enddo
          end if ! (if kida=1)
        ! READ IN 'WEIRD' REACTION RATES
        else if (REACTYPE(J) .EQ. 'WEIRD') then
          read(9,*)
          !print *, "Weird. Skipped this line" !move to the next line in the .rx file
          do I=1,NZ
            !H2 + O -> OH + H
            ...
            if (CHEMJ(1,J).EQ.'HO2'.AND.CHEMJ(2,J).EQ.'HO2') THEN
              A(J,I) = 2.3E-13*EXP(590./T(I)) +
     &                 1.7E-33*EXP(1000./T(I))*DEN(I)  !JPL-02
            endif
     ...
     $         CHEMJ(3,J).EQ.'CLO3') THEN
          ...
          !      A362_0=1.00D-32*T(i)**(-2.0)
          !      A362_inf=5.00D-12*T(i)**(-2.0)



          !       A(J,I) = TBDY(A362_0,A362_inf, 0.0E0,0.0E0,T(I),DEN(I))   ! from yuk (214)

          ! temptest
                 A(J,I) = 0.0   !zeroing for now. eventually rebuild this as CLO.O2

          !       A(J,I) = TBDY(1.0D-32,5.0E-12, 2.0E0,2.0E0,T(I),DEN(I))   ! from yuk (214)
                endif
          ...
          !       A(J,I) =  1E-31*Den(i)     !lowering this uncertain rate by a factor of 30 1e-13 is nominial case
                endif
     ...
     $                    /A367_EQ/1E5/DEN(I)
          ...
          !       print *, I, A(J,I),Alow,Ainf,T(I),DEN(I)
                endif
     ...
     $             CHEMJ(3,J).EQ.'CH3SH') THEN
          ...
          ! can't parse the Zhang paper so not sure what is correct.
                 A(J,I) = 4.81E-12 * EXP(-1100./T(I)) *
     &                    (T(I)/300.)**1.70    ! theory - Zhang et al. 2005
                endif
     ...
     $                 CHEMJ(3,J).EQ.'H2') THEN
                       A(J,I) = 8.34E-12 * EXP(-2212./T(I)) *
     &                          (T(I)/300.)**1.60 ! theory - Zhang et al. 2005
                    endif
     ...
     $          CHEMJ(3,J).EQ.'NO') )THEN
                  A(J,I) = 1.04E-6 * EXP(28618/T(I))*
     &                     (T(I)/298.)**(-1.61)*DEN(I) !Tsang 1986
                endif
          ...
          !   CH + H2 -> CH3            !apparantly the same as C+H2->CH23
                if (CHEMJ(1,J).EQ.'CH'.AND.CHEMJ(2,J).EQ.'H2'.AND.
     &          CHEMJ(3,J).EQ.'CH3') THEN
                 B0 = 8.75E-31 * EXP(524./T(I))
                 BI = 8.3E-11
                A(J,I) = B0*BI*DEN(I)/(B0*DEN(I) + BI)
          ...
          !   CH23 + H -> CH3
                if (CHEMJ(1,J).EQ.'CH23'.AND.CHEMJ(2,J).EQ.'H'.AND.
     &          CHEMJ(3,J).EQ.'CH3') THEN
     ...
     &          (CHEMJ(1,J).EQ.'C3H6'.AND.CHEMJ(2,J).EQ.'H')) THEN
          ...
          !  C2H2 + OH -> CH2CO + H
                if (CHEMJ(1,J).EQ.'C2H2'.AND.CHEMJ(2,J).EQ.'OH'.AND.
     &          CHEMJ(3,J).EQ.'CH2CO') THEN
          ...
          !  C3H8 + O -> C3H7 + OH
                if (CHEMJ(1,J).EQ.'C3H8'.AND.CHEMJ(2,J).EQ.'O') THEN
                  A(J,I)=1.6E-11 * EXP(-2900./T(I)) +
     &                   2.2E-11 * EXP(-2200./T(I))
                endif
          ...
          !  C2H3 + C2H5 -> CH3 + C3H5              WEIRD                                                            ! Romani et al. [1993]
                if (CHEMJ(1,J).EQ.'C2H3'.AND.CHEMJ(2,J).EQ.'C2H5'.AND.
     &          CHEMJ(3,J).EQ.'CH3') THEN
                  B0 = 1.9E-27
                  BI = 2.5E-11
          ...
          !  C3H5 + H -> C3H6                        WEIRD                                                            ! Yung et al. [1984] !has another branch!
                if (CHEMJ(1,J).EQ.'C3H5'.AND.CHEMJ(2,J).EQ.'H'.AND.
     &          CHEMJ(3,J).EQ.'C3H6') THEN
                  B0 = 1.E-28
                  BI = 1.E-11
                  A(J,I) = B0*BI*DEN(I)/(B0*DEN(I) + BI)
                endif
     ...
     &          (CHEMJ(1,J).EQ.'C3H3'.AND.CHEMJ(2,J).EQ.'H')) THEN
          ...
          !  C2H5 + H -> C2H6                        WEIRD                                                            ! Gladstone [1983]
                if (CHEMJ(1,J).EQ.'C2H5'.AND.CHEMJ(2,J).EQ.'H'.AND.
     &          CHEMJ(3,J).EQ.'C2H6') THEN
                  B0 = 5.5E-23/T(I)**2 * EXP(-1040./T(I))
                  BI = 1.5E-13 * EXP(-440./T(I))
                  A(J,I) = B0*BI*DEN(I)/(B0*DEN(I) + BI)
                endif

          end do ! loop over I (altitude)
        else
      ...
      real*8 FUNCTION ATBDY(A0,B0,G0,AI,BI,GI,T,D)
      implicit real*8(A-H,O-Z)
      R0 = A0*((T/300)**B0)*EXP(-G0/T)
        !print*,R0
      RI = AI*((T/300)**BI)*EXP(-GI/T)
      !PRINT*,RI
      PR=(R0*D)/RI
      !PRINT*,PR
      Y=((R0*D)/(1+((R0*D)/RI)))
      !PRINT*,Y
      C=-0.4-(0.67*LOG10(0.64))
      !PRINT*,C
      CN=0.75-(1.27*LOG10(0.64))
      !PRINT*,CN, "im cn"
      X=1/(1+((LOG10(PR))/(CN-0.14*LOG10(PR)+C))**2)
      !PRINT*,X, "im x"
      ATBDY=Y*(0.64)**X
      !PRINT*,ATBDY, "=atbdy"
      RETURN
