...
!!GNA EDITED ACCORDING TO COMMENT ABOVE ABOUT VTURB AND WFALL!!
       if (i.eq.1) then
          WNF = 0.5*((WFALL(I,K)+vturb)*DEN(I)*USOL(LL,I)
     2  + WFALL(I+1,K)*DEN(I+1)*USOL(LL,I+1))
       else
          WNF = 0.5*((WFALL(I,K))*DEN(I)*USOL(LL,I)
     2  + WFALL(I+1,K)*DEN(I+1)*USOL(LL,I+1))
          endif
...
c  but its not entirely clear that the boundary condition knows this 
      DO 15 K=1,NQ1
      FLOW(K) = FLUXO(K,1) - (YP(K,1) - YL(K,1)*SL(K,1))*DZ(1) !bottom
      FUP(K) = FLUXO(K,NZ1) + (YP(K,NZ) - YL(K,NZ)*SL(K,NZ))*DZ(NZ) !top
...
c similar to water we need special conditions for HCAER? 
...
C compute net model redox
      ...
      do i=1,NQ1  ! adding up total oxidants and reducants in atmosphere 
