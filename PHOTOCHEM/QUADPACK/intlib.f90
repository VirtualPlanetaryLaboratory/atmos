...
!  Parameters:
!
!    Input, integer ( kind = 4 ) ( kind = 4 ) NTAB, the number of entries in XTAB and
...
!    Output, real ( kind = 8 ) RESULT, the approximate value of the integral.
!
!    Output, integer ( kind = 4 ) ( kind = 4 ) IND, reliability indicator.
...
!
!    Input, integer ( kind = 4 ) ( kind = 4 ) KIND, specifies which polynomial is to be handled:
...
!
!    Input, integer ( kind = 4 ) ( kind = 4 ) N, specifies the number of coefficients to
!    calculate.
!
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) ( kind = 4 ) NTAB, the number of entries in FTAB and
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) ( kind = 4 ) NTAB, the number of tabulated points.
...
!
!    Input, integer ( kind = 4 ) ( kind = 4 ) IA, the entry of XTAB at which integration
!    is to begin.  IA must be no less than 1 and no greater
!    than NTAB.
!
!    Input, integer ( kind = 4 ) ( kind = 4 ) IB, the entry of XTAB at which integration
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) ( kind = 4 ) NTAB, the number of data points.
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) ( kind = 4 ) NTAB, the number of data points, including the
...
!
!    GAMMA(0.5) = SQRT(PI)
!    If N is a positive integer ( kind = 4 ), GAMMA(N+1) = N!, the standard factorial.
!
!  Discussion:
!
...
!
!    Output, integer ( kind = 4 ) ( kind = 4 ) IERR, a status code.
!    Normal Codes:
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) ( kind = 4 ) N, is the order of the matrix.
!
...
!    eigenvalues.
!
!    Output, integer ( kind = 4 ) ( kind = 4 ) IERR.
!    0, for normal return,
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) ( kind = 4 ) KIND, chooses the quadrature rule to be calculated.
...
!         -1 < ALPHA
!
!    Input, integer ( kind = 4 ) ( kind = 4 ) NORDER, the number of points used for the quadrature rule.
!
...
!
!    Input, integer ( kind = 4 ) ( kind = 4 ) KPTS, is normally zero.
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) ( kind = 4 ) NTAB, number of data points.
!
...
!    of workspace required is two times the number of integer ( kind = 4 )
!    divisors of NTAB-1.
!
...
!    Output, real ( kind = 8 ) RESULT, the approximate value of the integral.
!
!    Output, integer ( kind = 4 ) ( kind = 4 ) IND, error return flag.
...
!    Input, real ( kind = 8 ) B, the upper limit of integration.
!
!    Input, integer ( kind = 4 ) ( kind = 4 ) N, the number of points to use.
!
!    Output, real ( kind = 8 ) RESULT, the computed value of the integral.
!
  implicit none
...
!     2 RESULT probably does not meet requested error tolerance.
!
!    Output, integer ( kind = 4 ) ( kind = 4 ) K, the number of function evaluations
...
!    Input, real ( kind = 8 ) RELERR, the relative error tolerance.
!
!    Input, integer ( kind = 4 ) ( kind = 4 ) NLEAST, the least number of times the integration
!    is to be carried out before the convergence test is made.
!    A value 3 <= NLEAST <= 15 is suggested.
!
!    Input, integer ( kind = 4 ) ( kind = 4 ) NMOST, the most number of times the
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) ( kind = 4 ) N, the number of values.
!
...
!
!    Input, integer ( kind = 4 ) ( kind = 4 ) IOP, method switch:
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) ( kind = 4 ) NTAB, number of data points.  NTAB must be at least 3.
!
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) ( kind = 4 ) NTAB, the number of data points.  NTAB must be at least 3.
!
...
!
!    Input, integer ( kind = 4 ) ( kind = 4 ) N, the index of the desired component.
!
  implicit none
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) ( kind = 4 ) NTAB, is the number of data points.  (NTAB-1) must be
!    divisible by 6.
!
