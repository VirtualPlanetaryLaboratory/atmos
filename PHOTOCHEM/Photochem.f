            SUBROUTINE Photochem(ICOUPLE,TSTOP,DTP,NSTEPSP,TIME,ncouple)
...
C
      INCLUDE 'INPUTFILES/parameters.inc'
...
c The next three files are used only when this model is coupled with the climate model (ICOUPLE=1)

      open(113, file='COUPLE/fromPhoto2Clima.dat')            ! To be used as input for the climate model (coupling)
      open(116, file='COUPLE/fromClima2Photo.dat')
      open(114, file='COUPLE/mixing_ratios.dat')
...
C - Seperating the out.dist file into seperate files
       ...
       open(67, file='PHOTOCHEM/out.aersol', status='UNKNOWN')
       open(68, file='PHOTOCHEM/out.tridag', status='UNKNOWN')
...
C ***** FILL UP CHEMICAL PRODUCTION AND LOSS MATRICES *****
      ...
      IF(NUML(I).GT.NMAX) THEN          ! quit; too many reactions  (seems unnecesary, but whatever)
         PRINT 300,I
         STOP
      END IF
...
C ***** READ THE INPUT DATAFILE *****
      ...
      IF (IRESET.eq.1) CALL RESET    ! Defines mixing ratios for modern earth
...
c-mc note - Shawn's code has some changes to h escape terms here. consider porting....

      if(USETD.EQ.0) then
      VDEP(LSO4AER) = VDEP0(LSO4AER) + WFALL(1,1)
      VEFF(LSO4AER) = VEFF0(LSO4AER) + WFALL(NZ,1)

      VDEP(LS8AER) = VDEP0(LS8AER) + WFALL(1,2)
      VEFF(LS8AER) = VEFF0(LS8AER) + WFALL(NZ,2)

      if(NP.ge.3) then
      VDEP(LHCAER) = VDEP0(LHCAER) + WFALL(1,3)
      VEFF(LHCAER) = VEFF0(LHCAER) + WFALL(NZ,3)

      if (NP.eq.4) then
      VDEP(LHCAER2) = VDEP0(LHCAER2) + WFALL(1,4)
      VEFF(LHCAER2) = VEFF0(LHCAER2) + WFALL(NZ,4)
      endif
      endif
...
!diddle main loop particles
      ...
      if(lcountso4.gt.0) then
        if (J.GT.1) then
         USOL(LSO4AER,J)=USOL(LSO4AER,J-1)*EXP(-WFALL(J,1)*DZ(J)/EDD(J))
       else
        USOL(LSO4AER,J)=-1
       endif 
      endif 
      ...
      if(lcounts8.gt.0) then
       if(J.GT.1) then
       USOL(LS8AER,J) = USOL(LS8AER,J-1) * EXP(-WFALL(J,2)*DZ(J)/EDD(J))
       else
       USOL(LS8AER,J)=-1
       endif   
      endif 
      ...
      if(lcountHC.gt.0) then
        if (J.GT.1) then
         USOL(LHCAER,J) = USOL(LHCAER,J-1)*EXP(-WFALL(J,3)*DZ(J)/EDD(J))
        else
         USOL(LHCAER,J)=-1  
        endif   
      endif 
      ...
      if(lcountHC2.gt.0) then
        if (J.GT.1) then
         USOL(LHCAER2,J)=USOL(LHCAER2,J-1)*EXP(-WFALL(J,4)*DZ(J)/EDD(J))
        else
         USOL(LHCAER2,J)=-1  
        endif   
      endif 
...
C
 ...
 881  format(5E17.8)
     ...
     $                  (RPAR(i,j),j=1,NP) 
         write(67,fmtstr) (AERSOL(i,j),j=1,NP),(WFALL(i,j),j=1,NP),
     $                  (RPAR(i,j),j=1,NP)
        enddo  
      ...
      do i=1,nz
       ...
       write(68,fmtstr)  (PARTICLES(i,j),j=1,np)  
      enddo
      endif
...
C Transfer O3 and H2O
         WRITE(113,254) Z(I),PRESS(I),USOL(LO3,I),USOL(LH2O,I),
     &                 USOL(LCH4,I),SL(LCO2,I)/DEN(I)
  254    FORMAT(1PE9.3,5(E10.2))
  255   CONTINUE
        close(113)
         IF (IRESET.eq.0) FAR=1.000E-02                  !Argon not normally calculated by photochem
         FCH4=USOL(LCH4,1)
         FC2H6=USOL(LC2H6,1)
         ...
         FNO2=USOL(LNO2,1)
         JCOLD=JTROP
         WRITE(114,102) FAR, FCH4, FC2H6, FCO2,
